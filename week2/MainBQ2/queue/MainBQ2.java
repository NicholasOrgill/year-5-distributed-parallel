...
class Enqueuer implements Runnable {
	...
	public void run() {
		...
		for (int i = 0; i < num_messages; i++) {
			...
			System.out.println(queue.queueStats());
			queue.enq(m);
		}
	}
}
...
interface Queue<T> {
	...
	public abstract String queueStats();
}
...
class BadQueue1<T> implements Queue<T> {
	...
	public synchronized void enq(T x) {
		try {
			while (head % QSIZE == (tail + 1 % QSIZE)) {
				wait();
			}
			items.set((tail++) % QSIZE, x);
			if (head % QSIZE != (tail + 1) % QSIZE) {
				notifyAll();
			}
		} catch (InterruptedException e) {
			e.printStackTrace();
	...
	public synchronized T deq() {
		...
		T item = items.get((head++) % QSIZE);
		notifyAll();
		return item;
	}

	public String queueStats() {
		return String.format("h%d t%d", head, tail);
	}
}
