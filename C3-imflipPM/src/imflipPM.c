...
int main(int argc, char **argv)
{
    ...
    case 'V':
        MTFlipFunc = MTFlipV;
        FlipFunc = FlipImageV;
        strcpy(FlipType, "Vertical (V)");
        break;
    case 'H':
        MTFlipFunc = MTFlipH;
        FlipFunc = FlipImageH;
        strcpy(FlipType, "Horizontal (H)");
        break;
    case 'W':
        MTFlipFunc = MTFlipVM;
        FlipFunc = FlipImageV;
        strcpy(FlipType, "Vertical (W)");
        break;
    case 'I':
        MTFlipFunc = MTFlipHM;
        FlipFunc = FlipImageH;
        strcpy(FlipType, "Horizontal (I)");
        break;
    default:
        printf("Flip option '%c' is invalid. Can only be 'V', 'H', 'W', or 'I'\n", Flip);
        printf("\n\nNothing executed ... Exiting ...\n\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (NumThreads > 1)
        printf("(%9.4f ms per thread).  ", TimeElapsed / (double)NumThreads);
